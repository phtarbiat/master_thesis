...
// File: _coder_disturbanceObserver_rigid_model_api.cpp
//
...
emlrtContext emlrtContextGlobal{
    ...
    "disturbanceObserver_rigid_model",                    // fFunctionName
...
static real_T (*b_emlrt_marshallIn(const emlrtStack *sp, const mxArray *u_k,
                                   ...
                                   const char_T *identifier))[3]
{
  ...
  thisId.bParentIsCell = false;
  y = c_emlrt_marshallIn(sp, emlrtAlias(u_k), &thisId);
  emlrtDestroyArray(&u_k);
  return y;
}
...
// Return Type  : void
//
void disturbanceObserver_rigid_model_api(const mxArray *const prhs[7],
                                         const mxArray **plhs)
{
  emlrtStack st{
      nullptr, // site
  ...
  uDisObs_k = (real_T(*)[3])mxMalloc(sizeof(real_T[3]));
  // Marshall function inputs
  emlrt_marshallIn(&st, emlrtAliasP(prhs[0]), "paramStruct", &paramStruct);
  h = emlrt_marshallIn(&st, emlrtAliasP(prhs[1]), "h");
  u_k = b_emlrt_marshallIn(&st, emlrtAlias(prhs[2]), "u_k");
  theta_k = b_emlrt_marshallIn(&st, emlrtAlias(prhs[3]), "theta_k");
  omega_k = b_emlrt_marshallIn(&st, emlrtAlias(prhs[4]), "omega_k");
  omega_pk = b_emlrt_marshallIn(&st, emlrtAlias(prhs[5]), "omega_pk");
  uDisObs_pk = b_emlrt_marshallIn(&st, emlrtAlias(prhs[6]), "uDisObs_pk");
  // Invoke the target function
  disturbanceObserver_rigid_model(&paramStruct, h, *u_k, *theta_k, *omega_k,
                                  *omega_pk, *uDisObs_pk, *uDisObs_k);
  // Marshall function outputs
  *plhs = emlrt_marshallOut(*uDisObs_k);
}
...
// Return Type  : void
//
void disturbanceObserver_rigid_model_atexit()
{
  emlrtStack st{
      nullptr, // site
      nullptr, // tls
  ...
  disturbanceObserver_rigid_model_xil_terminate();
  disturbanceObserver_rigid_model_xil_shutdown();
...
//
// File trailer for _coder_disturbanceObserver_rigid_model_api.cpp
//
// [EOF]
//
